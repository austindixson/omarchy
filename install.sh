...
# is already installed, so the ARM repo has to be added now: otherwise herdr
# builds zig0.15 from source for two hours and aarch64 rejects it anyway.
ensure_arm_package_repo() {
  grep -q '^\[omarchy-aarch64\]' /etc/pacman.conf && return 0

  local block
  block=$(sed -n '/^\[omarchy-aarch64\]/,/^Server[[:space:]]*=/p' \
    "$checkout/default/pacman/pacman-stable-aarch64.conf")
  [[ -n $block ]] || fail "default/pacman/pacman-stable-aarch64.conf has no [omarchy-aarch64] section."

  if [[ ! -f /etc/pacman.d/mirrorlist.asahi-alarm ]]; then
    sudo cp -f "$checkout/default/pacman/mirrorlist.asahi-alarm" /etc/pacman.d/mirrorlist.asahi-alarm
  fi
...
# Swap x86_64-only package names for the aarch64 equivalent when one exists.
package_for_arch() {
  local package="$1"

  case $package in
    mise-bin) printf 'mise' ;;
    *) printf '%s' "$package" ;;
  esac
}

install_default_package_set() {
  local package resolved skipped=() unbuildable=() attempt_unavailable=0
  ...
  while read -r package; do
    ...
    resolved=$(package_for_arch "$package")
    yay -S --needed --noconfirm "$resolved" </dev/null || skipped+=("$resolved")
